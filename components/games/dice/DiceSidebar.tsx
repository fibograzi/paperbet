...
export default function DiceSidebar({ state, onDismissNudge, onDismissReminder }: DiceSidebarProps) {
  ...
  // Filtered casinos for Dice
  const diceCasinos = useMemo(
    () => CASINOS.filter((c) => c.games.includes("dice")).slice(0, 3),
    [],
  );

  const topCasino = diceCasinos[0] ?? null;
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      <div
        ...
        style={{
          borderColor: "#14B8A6",
          borderWidth: sessionRollCount >= 25 ? "2px" : "1px",
          boxShadow: sessionRollCount >= 25 ? "0 0 16px rgba(20,184,166,0.15)" : "none",
        }}
      >
        ...
        {showSessionReminder && (
          ...
          >
            <p className="text-xs text-pb-text-secondary">
              You&apos;ve played {sessionRollCount} rounds. Remember, this is practice mode.
            </p>
            <button
              type="button"
